...
def render_dashboard(instrument: str, price: float, result: Dict, capital: float):
    ...
    if result["decision"] in {"ACHAT","VENTE"} and m15.get("available"):
        setup=build_setup(price,m15.get("indicators",{}).get("atr14"),m15.get("structure_data",{}),result["decision"],m15.get("zones",{}))
    ...
    if not setup and result["decision"] in {"ACHAT","VENTE"} and not m15.get("available"):
        ...
        if h1.get("available"):
            setup_source="H1 (fallback)"; setup=build_setup(price,h1.get("indicators",{}).get("atr14"),h1.get("structure_data",{}),result["decision"],h1.get("zones",{}))
            if not setup.get("valid"): setup=None
